...
class VocabRepository (
    ...
    fun getWordCategories(wordCategories: String): LiveData<Result<List<DataItemWord?>?>> = liveData {
        emit(Result.Loading)
        try {
    ...
    fun translate(translateRequest: TranslateRequest): LiveData<Result<DataTranslate?>?> = liveData {
        emit(Result.Loading)
        try {
